...
- image_prompt: MINIMUM ~120 words. Production-ready STILL frame for live-action / photoreal film (NOT a comic page). START with the UNIQUE action, pose, and composition for THIS panel only (order N) — each panel must look like a different moment. Then include: (1) which cast members appear (must match the characters array) and their COMPLETE visual design copied from CHARACTERS (materials, colors, proportions, wardrobe, expression); (2) the SAME locked set (architecture, materials, layout) with a NEW camera setup — photoreal practical location, not CGI or soundstage; (3) lens/framing for shot_type; (4) same art direction as director bible. Repeat the same character DESIGN and SET architecture wording across shots — never repeat the same pose, blocking, or framing. NEVER describe captions, speech bubbles, thought bubbles, subtitles, title cards, sound-effect lettering, or any on-image text — dialogue is spoken later in video/audio; show speaking only via face, mouth, and body language.
- video_prompt: MINIMUM ~80 words. Motion-only delta on the still: camera_move, subject action, lighting shifts — do NOT introduce new characters or redesign anyone. Do NOT mention on-screen text or comic lettering.
- negative_prompt: comma-separated forbidden elements. ALWAYS include: no text overlay, no caption, no speech bubble, no subtitle, no readable text, no comic lettering, plus watermark, blurry, wrong species, extra characters, style drift.${animalRules}
