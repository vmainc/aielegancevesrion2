...
export const TARGET_LENGTH_OPTIONS: { value: ProjectTargetLength; label: string; hint: string }[] = [
  ...
  { value: 'short', label: 'Short film / web', hint: '~1–15 min' },
  { value: 'music_video', label: 'Music video', hint: '~2–6 min · video only' },
...
export function targetLengthModelGuidance (length: ProjectTargetLength | undefined): string {
  ...
  const map: Record<ProjectTargetLength, string> = {
    ...
    short:
      'Runtime target: about 1–15 minutes (short film, web pilot). Keep scope tight; a few scenes; avoid full three-act feature sprawl.',
    music_video:
      'Runtime target: music video (~2–6 minutes typical). Visual storytelling only — performance, mood, and beat-driven cuts; no dialogue scenes unless lip-sync is essential. Scope is montage-friendly; external music will be added in edit.',
    episode:
