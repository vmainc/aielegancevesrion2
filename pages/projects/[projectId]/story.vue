    ...
    <template v-else>
      ...
      >
        ...
        <p class="text-xs text-gray-600 mb-3">
          Used when you generate script or treatment below.
        </p>
        <div
        ...
        <p v-else class="text-sm text-amber-900 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2">
          No synopsis yet. Add your idea on the Story step (or import a script) before generating here.
        </p>
      </div>
      ...
      <div>
        ...
        <p class="text-sm text-gray-500 mb-4">
          Generate a prose treatment or a screenplay draft from the synopsis above and director notes.
...
watch(
  ...
  (p) => {
    ...
    if (!id) return
    if (isScratchWorkflowProject(p)) {
      void navigateTo(`/projects/${id}/overview`, { replace: true })
      return
    }
    if (projectStorySatisfiedByScriptImport(p)) {
      void navigateTo(`/projects/${id}/characters`, { replace: true })
    }
  },
  { immediate: true }
)
