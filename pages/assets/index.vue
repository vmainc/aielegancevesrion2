<template>
  <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
    <div class="mb-10">
      <h1 class="font-display text-3xl sm:text-4xl text-ivory tracking-wide mb-2">Assets</h1>
    ...
    <ul class="grid gap-4 sm:grid-cols-2 mb-10">
      <li v-for="card in cards" :key="card.to">
        <NuxtLink
          :to="card.to"
          class="block rounded-xl border border-gray-200 bg-studio-slate shadow-sm hover:border-primary/50 hover:bg-gray-50 transition-all p-6 h-full"
        >
      ...
      <div v-if="isAuthenticated" class="mb-10 rounded-xl border border-gray-200 bg-gray-50/80 p-5 sm:p-6">
        ...
        <div v-else-if="projectGroups.length" class="space-y-4">
          <details
            ...
            :key="g.key"
            class="rounded-lg border border-gray-200 bg-studio-slate overflow-hidden group"
          >
            <summary
